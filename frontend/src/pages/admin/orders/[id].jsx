              ...
              <div className='lg:col-span-2 space-y-6'>
                <div className='bg-slate-900 border border-slate-800 rounded-2xl p-6'>
                    ...
                    {order.items?.map((item, idx) => (
                      ...
                      >
                        <div className='w-16 h-16 bg-slate-700 rounded-lg overflow-hidden flex-shrink-0'>
                          {item.productId?.thumbnail || item.productId?.images?.[0]?.url ? (
                            <img
                              src={item.productId?.thumbnail || item.productId?.images?.[0]?.url}
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <p className='text-gray-500 text-sm'>
                            Qty: {item.quantity} × ${item.priceAtTime?.toFixed(2) || item.price?.toFixed(2)}
                          </p>
                        </div>
                  ...
                  <div className='mt-6 pt-6 border-t border-slate-700 space-y-2'>
                    ...
                    {(order.pricing?.discount > 0 || order.coupon?.code || order.pricing?.couponCode) && (
                        ...
                        <span className='flex items-center gap-2'>
                          ...
                          {(order.coupon?.code || order.pricing?.couponCode) && (
                            <span className='px-2 py-0.5 bg-green-500/10 border border-green-500/20 rounded text-xs'>
                              {order.coupon?.code || order.pricing?.couponCode}
                              {order.coupon?.discountPercent && ` (${order.coupon.discountPercent}%)`}
                            </span>
                          )}
                        </span>
                        <span>-${(order.pricing?.discount || order.coupon?.discountAmount || 0).toFixed(2)}</span>
                      </div>
                    )}
                    <div className='flex justify-between text-gray-400'>
                      <span>Shipping</span>
                      <span>${order.pricing?.shipping?.toFixed(2) || order.shipping?.cost?.toFixed(2) || '0.00'}</span>
                    </div>
                    <div className='flex justify-between text-gray-400'>
                      <span>Tax</span>
