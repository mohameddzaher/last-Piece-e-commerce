...
export default function OrderDetail() {
  ...
  const orderStatuses = ['pending', 'approved', 'dispatching', 'in_transit', 'delivered', 'completed'];
                ...
                {orderStatuses.map((status, index) => {
                  ...
                  return (
                    <div key={status} className='flex flex-col items-center'>
                      ...
                      <span className={`mt-2 text-xs capitalize ${isCompleted ? 'text-blue-400' : 'text-gray-500'}`}>
                        {status.replace('_', ' ')}
                      </span>
                    </div>
                  );
                ...
                {order.items?.map((item, index) => (
                  <div key={index} className='p-4 flex gap-4'>
                    ...
                    <div className='flex-1'>
                      <h3 className='font-medium text-white'>{item.product?.name || 'Product'}</h3>
          ...
          <div className='space-y-6'>
            ...
            <div className='bg-slate-900 rounded-2xl border border-slate-800 p-6'>
              ...
              <div className='space-y-3'>
                <div className='flex justify-between text-gray-400'>
                  <span>Subtotal</span>
                  <span className='text-white'>${order.totals?.subtotal?.toFixed(2)}</span>
                </div>
                ...
                <div className='flex justify-between text-gray-400'>
                  <span>Tax</span>
                  <span className='text-white'>${order.totals?.tax?.toFixed(2)}</span>
                </div>
                <div className='flex justify-between text-gray-400'>
                  <span>Shipping</span>
                ...
                <div className='flex justify-between text-lg font-bold'>
                  ...
                  <span className='text-white'>${order.totals?.total?.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div className='bg-slate-900 rounded-2xl border border-slate-800 p-6'>
              ...
              <p className='text-sm text-gray-500 mt-1'>
                Status: <span className='text-green-400 capitalize'>{order.payment?.status || 'paid'}</span>
              </p>
            </div>
          </div>
        </div>
